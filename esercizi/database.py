...
# Insert data
c1 = Customer(name='Maurizio Tolomeo',
              email='maurizio.tolomeo@gmail.it', address='via Bresso 3, Altamura')
session.add(c1)

c2 = Customer(name='Test Customer 1',
              email='test.customer1@gmail.it', address='via Romagna 32, Roma')
session.add(c2)

c3 = Customer(name='Test Customer 2', email='test.customer2@gmail.it',
              address='piazza Fontana 2, Torino')
session.add(c3)

c4 = Customer(name='Test Customer 3',
              email='test.customer3@gmail.it', address='via Roma 1, Bologna')
session.add(c4)
...
rows = session.query(Customer).from_statement(
    text("SELECT * FROM customers WHERE address LIKE 'piazza%'")).all()
for row in rows:
    print(row.id, row.name, row.email, row.address)
